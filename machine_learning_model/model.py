...
if __name__ == "__main__":
    ...
    d_newbalanceDest.drop_duplicates().to_json(
        'dict_newbalanceDest.json', orient='records')

    df = pandas.concat([df, pandas.get_dummies(df['device'])],
                       axis=1).drop('device', axis=1)
    df = pandas.concat([df, pandas.get_dummies(df['newbalanceDest'])], axis=1).drop(
